...
export function ChatInterface() {
  ...
  //     if (result.code === "00000" && result.data?.message) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Call backend API
      const response = await fetch('http://localhost:8080/message/model', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Check if response is successful
      if (result.code === "00000" && result.data?.output.content) {
        const aiResponse: Message = {
          id: (Date.now() + 1).toString(),
          content: result.data.output.content,
          role: "assistant",
          timestamp: new Date(),
        }
        setMessages((prev) => [...prev, aiResponse])
